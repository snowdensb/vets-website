...
fs.readdirSync('./src/applications/vaos/scripts/clinics').forEach(f => {
  ...
  const covidClinics = clinics.data.filter(clinic => {
    return clinic.nationalChar4 === 'CDQC' && (!!clinic.reactivationDate || !clinic.inactivationDate);
  });

  matchingClinics = matchingClinics.concat(covidClinics);
});

const header = `facility id,clinic id,name,friendly name, primary stop code, secondary stop code, display to patients, direct scheduling`;
fs.writeFileSync(
  './clinics.csv',
  [header]
    .concat(
      matchingClinics.map(row => {
        return `${row.sta6aid || row.facilityId},${row.locationIEN || ''},${row.locationName},${row.patientFriendlyLocationName || ''}, ${row.primaryStopCode}, ${row.secondaryStopCode}, ${row.displayAppointment}, ${row.directSchedulingPatient}`
      }),
    )
    .join('\n'),
);